...
from .. icaros  .correction_functions import normalization
from .. icaros  .correction_functions import apply_3Dmap
...
def test_apply_3Dmap(dummy_map):
    Ec = apply_3Dmap(dummy_map, NormMethod.maximum, dummy_map.dt, dummy_map.x, dummy_map.y, dummy_map.mu, unit = keV)
    assert len(Ec) == len(dummy_map)


def test_apply_3Dmap_same_values(dummy_map):
    dummy_map['mu'] = 8000
    Ec = apply_3Dmap(dummy_map, norm_method = NormMethod.maximum, dt = dummy_map.dt, x = dummy_map.x, y = dummy_map.y, E = dummy_map.mu, unit = None)
