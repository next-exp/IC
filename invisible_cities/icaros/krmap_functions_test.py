...
from .. core     .core_functions  import fix_random_seed
from .. core     .testing_utils   import assert_dataframes_close
from .. icaros   .krmap_functions import create_empty_map
from .. icaros   .krmap_functions import get_median
from .. icaros   .krmap_functions import gaussian_fit_ready
from .. icaros   .krmap_functions import gaussian_fit
from .. icaros   .krmap_functions import fit_map
from .. icaros   .krmap_functions import merge_maps
from .. icaros   .krmap_functions import include_coordinates
from .. icaros   .krmap_functions import gauss_seed
from .. icaros   .krmap_functions import quick_gauss_fit
from .. icaros   .krmap_functions import save_map
from .. icaros   .krmap_functions import compute_metadata
from .. icaros   .krmap_functions import get_time_evol_single_slice
...
def test_gaussian_fit_few_entries(dummy_empty_df):
    result_fit = gaussian_fit(dummy_empty_df.S2e, 1, min_events = 10)
...
def test_gaussian_fit_Nevents():
    ...
    N = len(df_test)
    result_fit = gaussian_fit(df_test.S2e, nbins = 6)
...
def test_gaussian_fit_computes_right_values():
    ...
    results = gaussian_fit(S2e_df.S2e, nbins = 50)
...
def test_fit_map_S2e_values():
    ...
    result = fit_map(df = df,
                          ...
                          fit_function = gaussian_fit_ready(20, 50))
...
def test_fit_map_missing_bins():
    ...
    result = fit_map(df = df,
                          ...
                          dt_nbins = dt_nbins,
                          S2e_range = (1000, 20000),
                          fit_function = gaussian_fit_ready)
...
def test_compute_3D_map():
    ...
    full_map = compute_3D_map(df_, xy_range = (-100, 100), dt_range = (20, 1350), xy_nbins = 10, dt_nbins = 10, S2e_range = (1000, 20000),fit_function = gaussian_fit_ready(20, 50))
...
def test_get_time_evol_single_slice_shape(dummy_get_time_evol):
   t_evol = get_time_evol_single_slice(dummy_get_time_evol, 1, 0, 0, SelRegionMethod.circle, 1000, np.linspace(1250, 1400, 50), (1000, 1350), np.linspace(30, 60, 101))

   assert t_evol.shape[1] == 42
...
def test_get_time_evol(dummy_get_time_evol):
    ...
    t_evols = get_time_evol(df2, 1, 1, 0, 0, SelRegionMethod.circle, 1000, np.linspace(1250, 1400, 50), (1000, 1350), np.linspace(30, 60, 101), error = False)
