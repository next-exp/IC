...
def test_create_deconvolution_df(ICDATADIR):
    ...
    for c in true_dst.columns:
        assert np.allclose(new_dst.loc[:, c].values, true_dst.loc[:, c].values)
...
def test_beersheba_exact_result3d(ICDIR, ICDATADIR, PSFDIR, config_tmpdir):
    PATH_IN     = os.path.join(ICDATADIR, "exact_Kr_tracks_with_MC.h5")
