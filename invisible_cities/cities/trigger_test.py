"""
code: trigger_test.py
description: test suite for trigger
author: P. Ferrario
...
from invisible_cities.cities.trigger import Trigger, TRIGGER
...
def conf_file_name(config_tmpdir):
    ...
    conf_file_name = str(config_tmpdir.join('trigger_test.conf'))
    Trigger.write_config_file(conf_file_name,
...
def test_command_line_trigger_krypton(conf_file_name, config_tmpdir, ICDIR):
    ...
    nevts, n_evts_run = TRIGGER(['TRIGGER',
                                 '-c', conf_file_name,
                                 '-i', PATH_IN,
                                 '-o', PATH_OUT,
                                 '-n', '20',
