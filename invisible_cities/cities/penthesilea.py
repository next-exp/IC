...
def penthesilea(files_in, file_out, compression, event_range, print_mod, run_number,
                ...
                Qthr_sipmhit,
                slice_cluster_reco_params = dict(),
                global_reco_params        = dict()):
    #  slice_cluster_reco_params are qth, qlm  , lm_radius  , new_lm_radius  , msipm   used for hits reconstruction using corona
    #  Qthr_sipmhit is threshold for 1 hit per sipm hits reconstruction 
    ...
    reco_algo_slice_cluster  = compute_xy_position(**slice_cluster_reco_params)
    build_cluster_hits       = df.map(hit_builder(run_number, drift_v, reco_algo_slice_cluster, rebin),
                                      args = ("pmap", "selector_output", "event_number", "timestamp" ),
                                      out  = "cluster_hits"                                          )
    
    
    reco_algo_slice_sipmhit  = compute_xy_position(Qthr=Qthr_sipmhit, Qlm=0*units.pes, lm_radius=0*units.mm, new_lm_radius=0*units.mm, msipm=1)
    build_sipm_hits          = df.map(hit_builder(run_number, drift_v, reco_algo_slice_sipmhit, rebin),
                                      args = ("pmap", "selector_output", "event_number", "timestamp" ),
                                      out  = "sipm_hits"                                             )
    ...
    with tb.open_file(file_out, "w", filters = tbl.filters(compression)) as h5out:
        ...
        write_corona_hits      = df.sink(         hits_writer(h5out, group_name = 'RECO'         ), args="cluster_hits"                              )
        write_sipm_hits        = df.sink(         hits_writer(h5out, group_name = 'RECO_sipmhits'), args="sipm_hits"                                 )
        ...
        return push(source = pmap_from_files(files_in),
                    pipe   = pipe(
                        ...
                        pmap_select          .filter          ,
                        event_count_out      .spy             ,
                        df.fork((build_cluster_hits           , write_corona_hits     ),
                                (build_sipm_hits              , write_sipm_hits       ),
