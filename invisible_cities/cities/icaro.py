...
from .. reco.corrections    import NormStrategy
...
from .. reco.icaro_components import KrFitFunction
from .. reco.icaro_components import get_number_of_bins, get_XY_bins, get_bin_counts_and_event_bin_id
from .. reco.icaro_components import create_df_kr_map, fit_and_fill_map, valid_bin_counter
from .. reco.icaro_components import regularize_map
...
def map_builder(detector_db, run_number, *map_params):
    ...
    def compute_map(dst          : pd.DataFrame,
                    ...
                    n_bins       : int,
                    n_min_in_bin : int,
                    r_max        : float, # Take from DB when possible
                    validity_par : float,
                    XYrange      : Tuple[float, float],  # Take from DB when possible
        ...
        -------
        kr_map : pd.DataFrame
            Map dataframe containing the obtained parameters: E0, LT, uncertainties, etc.
        '''
        ...
        counts, bin_labels = get_bin_counts_and_event_bin_id(dst = dst, bins = bins)
        ...
        valid_bin_counter(map_df = kr_map, validity_parameter = validity_par)

        regularized_map = regularize_map(maps = kr_map, x2range = x2range)
...
#                      norm_strat : norm_strategy   = NormStrategy.max):
#     """
