...
from . ic_types_c        import minmax as minmax_c
from . ic_types          import xy
from . ic_types_c        import xy     as xy_c
...
@mark.parametrize("minmax_",
                  (minmax, minmax_c))
@given(sensible_floats, sensible_floats)
def test_minmax_does_not_accept_min_greater_than_max(minmax_, a, b):
...
@mark.parametrize("minmax_",
                  (minmax, minmax_c))
@given(sensible_floats, sensible_floats)
def test_minmax_getitem(minmax_, lo, hi):
    lo, hi = sorted([lo, hi])
    mm = minmax_(lo, hi)
